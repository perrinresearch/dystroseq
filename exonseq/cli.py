...
from .exon_cli import main as exon_main
from .report_generator import generate_reproducibility_report, get_version_string, print_version_info


def parse_args() -> argparse.Namespace:
	p = argparse.ArgumentParser(description="exonSeq - Export DNA, RNA, and protein sequences with exon mapping from Ensembl")
	subparsers = p.add_subparsers(dest='command', help='Available commands')
	
	# Main export command
	export_parser = subparsers.add_parser('export', help='Export gene sequences and exon mapping')
	export_parser.add_argument("--species", required=True, help="Ensembl species name, e.g., homo_sapiens")
	export_parser.add_argument("--gene", required=True, help="Gene symbol, e.g., DMD")
	export_parser.add_argument("--reference", required=False, default=None, help="Reference genome coord system version, e.g., GRCh38")
	export_parser.add_argument("--coords", required=False, default=None, help="Genomic region CHR:START-END:STRAND for DNA export")
	export_parser.add_argument("--transcript", required=False, default=None, help="Optional Ensembl transcript ID override")
	export_parser.add_argument("--variant", required=False, default=None, help="Variant notation (e.g., chrX:g.123456-789012del, c.123-456_789+012dup)")
	export_parser.add_argument("--out", required=True, help="Output directory")
	export_parser.add_argument("--no-report", action="store_true", help="Skip generating reproducibility report")
	
	# Add version argument to main parser
	p.add_argument("--version", action="store_true", help="Show version information")
	
	# Exon utilities command
	exon_parser = subparsers.add_parser('exon', help='Exon utilities (convert, fasta-range, fasta-list, list, info)')
	exon_parser.add_argument('exon_args', nargs=argparse.REMAINDER, help='Arguments for exon utilities')
...
def main() -> None:
	...
	if args.command == 'exon':
		# Handle exon utilities
		import sys
		# Create a new sys.argv with the exon command and its arguments
		original_argv = sys.argv[:]
		sys.argv = ['exonseq'] + args.exon_args
		try:
			exon_main()
		finally:
	...
	if not getattr(args, 'no_report', False):
		try:
			...
			report_path = generate_reproducibility_report(command_args, Path(args.out), "export")
			print(f"\nReproducibility report generated: {report_path}")
		except Exception as e:
			print(f"Warning: Could not generate reproducibility report: {e}")
